...
export class ConfigStore {
  ...
  get() {
    ...
    return {
      ...
      tallyColors: {
        active: String(value?.tallyColors?.active ?? '#ff0000'),
        belowThreshold: String(value?.tallyColors?.belowThreshold ?? '#00ff00'),
        muted: String(value?.tallyColors?.muted ?? '#00008b')
      },
