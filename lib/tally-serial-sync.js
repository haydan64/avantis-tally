...
export async function scanTallyBeacons({
  ...
  async function scanSinglePath(path) {
  ...
  const results = [];
  const maxConcurrency = Math.max(1, Math.floor(Number(concurrency) || 4));
  let nextIndex = 0;

  const workers = Array.from({ length: Math.min(maxConcurrency, paths.length) }, async () => {
    while (true) {
      ...
      results[currentIndex] = await scanSinglePath(paths[currentIndex]);
    }
  });
