...
export class AvantisClient extends EventEmitter {
  constructor(options = {}) {
    ...
    this.reconnectIntervalMs = Math.max(500, Number(options.reconnectIntervalMs) || 2000);
  ...
  scheduleReconnect() {
    if (!this.shouldReconnect || this.connected || this.connecting || this.reconnectTimer || !this.host) {
      return;
    }
  ...
  startConnectionAttempt() {
    if (!this.shouldReconnect || this.connected || this.connecting || !this.host) {
      return;
    }
  ...
  connect(host, port = 51325) {
    ...
    this.shouldReconnect = this.host.length > 0;
    this.clearReconnectTimer();
    ...
    if (this.socket) {
      try {
        ...
        // Ignore destroy errors while reconnecting.
      }
      this.socket = null;
    }
